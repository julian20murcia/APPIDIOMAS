...
function seasonId(date = new Date()): string {
...
function sanitizeQuestion(question: QuestionDocument) {
  return {
    id: question.id,
    type: question.type,
...
export const getCompetitionSeason = onCall(
  {
    region: REGION,
...
export const startCompetitiveSession = onCall(
  {
    region: REGION,
    enforceAppCheck: true,
    timeoutSeconds: 30,
  ...
  async (request) => {
    ...
    if (!Number.isInteger(lessonNumber) || lessonNumber < 1 || lessonNumber > 36) {
      throw new HttpsError(
        'invalid-argument',
        'La lección debe estar entre 1 y 36.',
      );
    }
    ...
    if (questionsSnapshot.size < QUESTION_COUNT) {
      throw new HttpsError(
        'failed-precondition',
        `La lección ${lessonNumber} no tiene suficientes preguntas publicadas.`,
      );
    }
...
export const submitCompetitiveSession = onCall(
  {
    region: REGION,
    enforceAppCheck: true,
    timeoutSeconds: 30,
  ...
  async (request) => {
    ...
    const submitted = Array.isArray(request.data?.answers)
      ? request.data.answers as SubmittedAnswer[]
      : [];

    if (!sessionId || submitted.length === 0) {
      throw new HttpsError(
        'invalid-argument',
        'La sesión y las respuestas son obligatorias.',
      );
    }
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (session.uid !== uid) {
        throw new HttpsError(
          'permission-denied',
          'Esta sesión pertenece a otro jugador.',
        );
      }
      ...
      if (expiresAt.toMillis() < Date.now()) {
        ...
        throw new HttpsError(
          'deadline-exceeded',
          'La sesión competitiva expiró.',
        );
      }

      const questions = session.questions as QuestionDocument[];
      const answersById = new Map(
        submitted.map((answer) => [answer.questionId, answer]),
      );
      ...
      for (const question of questions) {
        ...
        if (correct) {
          ...
          const responseMs = Math.max(
            0,
            Math.min(Number(answer.responseMilliseconds) || 0, 180000),
          );
      ...
      const score = totalBasePoints === 0
        ...
        : Math.max(
            ...
            Math.min(100, Math.round(
              (earnedBasePoints / totalBasePoints) * 100,
            )),
          );

      const passed = score >= 70;
      const difficultyXp = questions.reduce((total, question) => {
        if (question.difficulty === 'hard') return total + 3;
        if (question.difficulty === 'medium') return total + 2;
        return total + 1;
      }, 0);
      ...
      const xp = Math.max(
        ...
        Math.min(
          250,
          Math.round(score * 0.8) + speedXp + difficultyXp + (passed ? 20 : 0),
        ),
      );
      ...
      const countryCode = String(
        request.auth?.token?.countryCode ?? previous.countryCode ?? 'CO',
